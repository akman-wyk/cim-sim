...
std::shared_ptr<ExecuteInsPayload> DecoderV2::decodePimIns(const InstV2& ins) const {
    ...
    if (auto op = ins.getOpcodeEnum(); op == +OPCODE::CIM_MVM) {
        ...
        p.op = ins.OSUM_MOV ? PimControlOperator::output_sum_move
               : ins.OSUM   ? PimControlOperator::output_sum
...
std::shared_ptr<ExecuteInsPayload> DecoderV2::decodeSIMDIns(const InstV2& ins) const {
    ...
    p.input_cnt = ((ins.opcode >> 2) & 0x11) + 1;

    int i1_addr = reg_unit_->readRegister(ins.rs, false);
    int i2_addr = (p.input_cnt < 2) ? 0 : reg_unit_->readRegister(ins.rt, false);
    ...
    p.opcode = static_cast<unsigned int>(ins.funct);
